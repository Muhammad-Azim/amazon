    ...
    #extract and record data item from a single record
        ...
        try:    
            ...
            def seller_info(comp_atag):
                ...
                for i in comp_atag:
                    ...
                    for value in product_list:
                        asin_array = ''
                        asin_array = value.get_attribute('data-asin').strip()
                        if (not(re.match("^[a-zA-Z0-9]*$", asin_array)) and asin_array == ''):
                            continue
                        else:
                            seller_product_asin.append(np.unique(asin_array))
                    ...
                    while condition:
                        try:
                            ...
                            product_list = driver.find_element_by_class_name('s-main-slot').find_elements_by_class_name('s-result-item')
                            for value in product_list:
                                seller_product_asin.append(value.get_attribute('data-asin').strip())
                        except:
                            condition = False
            ...
            seller_info(comp_atag)    
               ...
               print('Competitors Link doesn`t exists')        
        print('asin lists')
        print(type(seller_product_asin[0]))
        print(seller_product_asin[0])
        #print(seller_product_asin.count)
        print('------')
        #time.sleep(5)
        
        df = pd.DataFrame(seller_product_asin)
        print(df.head(20))
        
        df.to_csv(r'asin.csv')
        

        ten_row = df[0].iloc[[0, 10]]
        seller_links = []
        
        print(ten_row) 
        
        
        def seller_data(ten_row):
            for i in ten_row:
                ...
                for j in seller_links:
                        ...
                        try:
                            driver.get('https://keepa.com/#!product/1-'+i)
                            ...
                            seller_keepa = re.findall('[0-9]+',avg)
                        except AttributeError:
                            seller_keepa = ''
                        except Exception:
                            print('Keepa Link doesn`t exists')    
                        ...
                        seller_result = (seller_product_asin,market_id,me_id,seller_url,seller_title,seller_price,seller_dimen,seller_keepa,seller_ranking,seller_rank,seller_competitors,seller_parent,seller_buy_box,seller_buybox_price,seller_atag,seller_rating)
                        #print(counter)
                        seller_records.append(seller_result)
                            
        seller_data(ten_row)
        print(seller_links[0])             
